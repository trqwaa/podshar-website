...
export async function Compare({ locale, userId }: { locale: Locale; userId: string }) {
  ...
  const verdict = (pick: (m: MemberDay) => number | null) => {
    const played = members
      .map((m) => ({ m, v: pick(m) }))
      .filter((x): x is { m: MemberDay; v: number } => x.v !== null);
    if (!played.length) return null;
    const sorted = [...played].sort((a, b) => b.v - a.v);
    return { best: sorted[0], worst: sorted.length > 1 ? sorted[sorted.length - 1] : null };
  };
  const dota = verdict((m) =>
    m.dota.totals.wins + m.dota.totals.losses ? (m.dota.totals.delta ?? 0) : null
  );
  const brawl = verdict((m) =>
    m.brawl.totals.wins + m.brawl.totals.losses ? (m.brawl.totals.delta ?? 0) : null
  );
    ...
    <div className="grid grid-cols-1 gap-3 sm:gap-4">
      <section className="block-card animate-rise-in flex flex-col gap-4 p-6 [animation-delay:60ms]">
        <div>
          ...
          <p className="ps-label mt-2">{t('compareHint')}</p>
        </div>
        ...
        {!dota && !brawl ? (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {[
              ...
              { label: t('views.brawl'), v: brawl, unit: (n: number) => t('trophiesUnit', { n }), estimated: false }
            ].map((g) =>
              g.v ? (
                <div key={g.label} className="rounded-sm bg-sunk px-4 py-3">
                  ...
                  <p className="mt-1 text-base text-ink">
                    ...
                    <span className="text-ink-muted">
                      {g.v.best.v > 0 ? t('leader') : t('leastBad')}:{' '}
                    </span>
                    <span className="font-semibold normal-case">{g.v.best.m.displayName}</span>{' '}
                    <Delta value={g.v.best.v} estimated={g.estimated} />{' '}
                    <span className="text-ink-muted">{g.unit(Math.abs(g.v.best.v))}</span>
                  </p>
                  {g.v.worst && g.v.worst.v < g.v.best.v ? (
                    <p className="text-base text-ink">
                      <span className="text-ink-muted">
                        {g.v.worst.v < 0 ? t('loser') : t('leastGood')}:{' '}
                      </span>
                      <span className="font-semibold normal-case">{g.v.worst.m.displayName}</span>{' '}
                      <Delta value={g.v.worst.v} estimated={g.estimated} />{' '}
                      <span className="text-ink-muted">{g.unit(Math.abs(g.v.worst.v))}</span>
                    </p>
                  ) : null}
                </div>
              ) : null
          ...
          >
            <header className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-lg font-semibold normal-case leading-tight text-ink">
                  {m.displayName}
                </p>
            ...
            <DayBlock
              ...
              day={m.dota.totals}
              badge={
                m.dota.profile?.medal ? (
                  <Medal medal={m.dota.profile.medal} stars={m.dota.profile.stars} size={32} />
                ) : null
              }
              ...
              badge={
                m.brawl.profile ? (
                  <span className="text-sm tabular-nums text-ink-muted">
                    {t('trophiesN', { n: m.brawl.profile.trophies })}
                  </span>
                ) : null
              }
            ...
            {chips.map((c) => (
              ...
              >
                ...
                <ResultChip
                  result={c.result}
                  letter={
                    c.result === 'WIN' ? letters[0] : c.result === 'LOSS' ? letters[1] : letters[2]
                  }
                  spoken={`${c.title}: ${spoken[c.result]}`}
                />
              </li>
            ))}
          </ul>
