...
import { boardFor, ownBrawlers } from '@/lib/games-board';
 ...
 */
export async function BrawlFull({ locale, userId, tab }: { locale: Locale; userId: string; tab: Tab }) {
  const [t, board, meta] = await Promise.all([
    getTranslations({ locale, namespace: 'gamesBoard' }),
    boardFor(userId),
    brawlMeta()
  ]);
  const mine = board.brawl.account ? await ownBrawlers(board.brawl.account.id) : [];

  const list = meta ? (tab === 'meta' ? brawlMetaList(meta).slice(0, 15) : tab === 'shame' ? brawlShameList(meta) : brawlHonourList(meta)) : [];
        ...
        <section className="block-card animate-rise-in flex flex-col gap-3 p-6 [animation-delay:120ms]">
          <div>
            ...
            <p className="ps-label mt-1 normal-case">
              {meta
                ? `${hint} · ${t('sampleBrawl', {
                    battles: meta.battles.toLocaleString(locale),
                    players: meta.players,
                    when: ago(meta.at, locale)
                  })}`
                : hint}
            </p>
          </div>
          {!meta ? (
            ...
            <ol className="flex flex-col">
              {list.map((b, i) => (
                <li key={b.id} className={`flex items-center gap-3 py-2.5 ${i > 0 ? 'ps-rule' : ''}`}>
                  <span className="w-6 shrink-0 text-right text-sm tabular-nums text-ink-faint">{i + 1}</span>
                  <BrawlerIcon brawler={b} size={32} />
                  <span className="min-w-0 flex-1 truncate text-base font-medium normal-case leading-tight text-ink">{b.name}</span>
                  <span className="hidden w-16 text-right text-sm tabular-nums text-ink-muted sm:block">{percent(b.pickRate)}</span>
                  <Pct value={b.winRate} className="w-16 text-right text-base" />
                </li>
              ))}
            </ol>
          )}
        ...
        <aside className="block-card animate-rise-in flex flex-col gap-3 self-start p-6 [animation-delay:180ms]">
          ...
          {tab === 'meta' && profile ? (
            <p className="text-base text-ink">
              <span className="font-semibold tabular-nums">{profile.trophies.toLocaleString(locale)}</span>{' '}
              <span className="text-ink-muted">
                {t('trophies')} · {t('peak', { peak: profile.highest })}
              </span>
              {profile.recentPlayed ? (
                <>
                  <br />
                  <Pct value={profile.recentWins / profile.recentPlayed} />{' '}
                  <span className="text-sm text-ink-muted">{t('recentWinrate', { played: profile.recentPlayed })}</span>
                </>
              ) : null}
            </p>
          ) : null}
          ...
          {ownList.length === 0 ? (
            <p className="text-base text-ink-faint">{board.brawl.account ? t('ownNone') : t('notLinked')}</p>
          ) : (
            <ul className="flex flex-col">
              {ownList.map((b, i) => {
                const known = brawler(b.id);
                return (
                  <li key={b.id} className={`flex items-center gap-3 py-2 ${i > 0 ? 'ps-rule' : ''}`}>
                    <BrawlerIcon brawler={known} size={28} />
                    <span className="min-w-0 flex-1 truncate text-base font-medium normal-case text-ink">{known.name}</span>
                    <span className="text-sm tabular-nums text-ink-muted">{t('ownGames', { games: b.games })}</span>
                    <Pct value={b.wins / b.games} className="text-sm" />
                  </li>
                );
              })}
            </ul>
          )}
          {seen ? <p className="ps-label">{t('brawlOwnNote', { n: seen })}</p> : null}
        </aside>
      </div>
    </div>
  );
