...
export function normalizeTag(input: string): string | null {
  // Срезаем решётку или её URL-кодировку `%23` — как **строки**. Здесь стоял
  // `/^[#%23]+/`, и квадратные скобки делали из него набор отдельных символов:
  // «любой из #, %, 2, 3, сколько угодно подряд». Тег `#2YP80CJ9J` терял и
  // решётку, и двойку, превращался в `YP80CJ9J` — валидный, но **чужой** — и
  // привязывался аккаунт постороннего человека. Двойка в начале тега у Supercell
  // обычное дело: из четырнадцати допустимых символов она одна из самых частых.
