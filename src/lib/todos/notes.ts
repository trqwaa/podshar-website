 ...
 */
/** Размер листочка по умолчанию, в долях доски. */
export const NOTE_W = 0.2;
export const NOTE_H = 0.26;

/** Пределы, до которых листочек можно тянуть за угол. */
export const MIN_W = 0.11;
export const MIN_H = 0.13;
export const MAX_W = 0.5;
export const MAX_H = 0.6;

export const clampW = (v: number) => Math.min(Math.max(v, MIN_W), MAX_W);
export const clampH = (v: number) => Math.min(Math.max(v, MIN_H), MAX_H);
...
/** След листочка на доске: где лежит и сколько занимает. */
export type Footprint = Spot & { w: number; h: number };

/**
 * Куда положить новый листочек, чтобы он не лёг поверх соседа.
 *
 * Свободным считается место, где **следы не пересекаются**, а не где центры
 * разошлись на глазок. Первая версия сравнивала координаты с шагом 0.05 при
 * ширине листочка 0.2 — и честно клала новую записку на предыдущую, считая
 * место свободным. Размер соседей берётся настоящий: растянутый за угол листочек
 * занимает больше, и делать вид, что все одинаковые, значит вернуть тот же баг.
 *
 * Обход идёт сеткой слева направо и сверху вниз: доска с рядами читается, доска
 * со случайной россыпью — нет. Если рядов не хватило, листочек ложится со
 * сдвигом: лучше внахлёст, чем нигде.
 */
export function freeSpot(taken: Footprint[]): Spot {
  ...
  const hits = (spot: Spot, other: Footprint) =>
    spot.x < other.x + other.w &&
    other.x < spot.x + NOTE_W &&
    spot.y < other.y + other.h &&
    other.y < spot.y + NOTE_H;

  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < cols; col += 1) {
      const spot = { x: 0.03 + col * gapX, y: 0.04 + row * gapY };
      if (!taken.some((other) => hits(spot, other))) return spot;
    }
  }
